...
type TableStore struct {
	sync.RWMutex
	data map[int64]models.TableInfo
}

// NewTableStore khoi tao data
func NewTableStore() *TableStore {
	return &TableStore{
		data: make(map[int64]models.TableInfo),
	}
}

// GetByID lay 1 table theo id trong data
func (ts *TableStore) GetByID(id int64) (tf models.TableInfo, err error) {
	ts.RLock()
	defer ts.RUnlock()
...
func (ts *TableStore) Set(tf models.TableInfo) {
	ts.RLock()
	defer ts.RUnlock()
