...
type PlayerStore struct {
	sync.RWMutex
	data map[int64]models.PlayerInfo
}

// NewPlayerStore khoi tao data Player
func NewPlayerStore() *PlayerStore {
	return &PlayerStore{
		data: make(map[int64]models.PlayerInfo),
	}
}

// GetByID lay Player theo id trong data
func (ts *PlayerStore) GetByID(id int64) (tf models.PlayerInfo, err error) {
	ts.RLock()
	defer ts.RUnlock()
...
func (ts *PlayerStore) Set(tf models.PlayerInfo) {
	ts.RLock()
	defer ts.RUnlock()
