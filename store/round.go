...
type RoundStore struct {
	sync.RWMutex
	data map[int64]models.RoundInfo
}

// NewRoundStore Khoi tao data round
func NewRoundStore() *RoundStore {
	return &RoundStore{
		data: make(map[int64]models.RoundInfo),
	}
}

// GetByID lay 1 round trong data
func (ts *RoundStore) GetByID(id int64) (tf models.RoundInfo, err error) {
	ts.RLock()
	defer ts.RUnlock()
...
func (ts *RoundStore) Set(tf models.RoundInfo) {
	ts.RLock()
	defer ts.RUnlock()
