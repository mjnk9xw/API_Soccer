...
type MatchStore struct {
	sync.RWMutex
	data map[int64]models.MatchInfo
}

// NewMatchStore khoi tao data Match
func NewMatchStore() *MatchStore {
	return &MatchStore{
		data: make(map[int64]models.MatchInfo),
	}
}

// GetByID lay 1 Match theo id trong data Match
func (ts *MatchStore) GetByID(id int64) (tf models.MatchInfo, err error) {
	ts.RLock()
	defer ts.RUnlock()
...
func (ts *MatchStore) Set(tf models.MatchInfo) {
	ts.RLock()
	defer ts.RUnlock()
