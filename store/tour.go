...
type TourStore struct {
	sync.RWMutex
	data map[int64]models.TourInfo
}

// NewTourStore khoi tao data Tour
func NewTourStore() *TourStore {
	return &TourStore{
		data: make(map[int64]models.TourInfo),
	}
}

// GetByID lay 1 TourInfo trong data
func (ts *TourStore) GetByID(id int64) (tf models.TourInfo, err error) {
	ts.RLock()
	defer ts.RUnlock()
...
func (ts *TourStore) GetAll() (lst []models.TourInfo, err error) {
	ts.RLock()
	defer ts.RUnlock()
	err = errSC.Success
	lst = make([]models.TourInfo, 0)
...
// Set them 1 TourInfo
func (ts *TourStore) Set(tf models.TourInfo) {
	ts.RLock()
	defer ts.RUnlock()
